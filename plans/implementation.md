...
Long-take lip sync/drift, 4K throughput, physical device removal during recording, and every desktop portal/backend combination still need device-specific checks. The short hardware test establishes capture and file validity; duration agreement alone does not establish lip sync. See the acceptance list in [the original plan](monologue.md).
